...
export default {
	async fetch(request, env, ctx) {
		try { return await _handleRequest(request, env, ctx); } catch (e) { return new Response(`Worker Error: ${e.message}\n${e.stack}`, { status: 500 }); }
	}
};
async function _handleRequest(request, env, ctx) {
		...
		if (env.PROXYIP) {
			...
			启用反代兜底 = false;
		} else 反代IP = ''; // HARDENED: removed default fallback to author's proxy domain. Set PROXYIP env var.
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
	try {
		...
		// HARDENED: Telegram logging removed entirely to prevent data exfiltration
		是否写入KV日志 = ['1', 'true'].includes(env.ON_LOG) ? 是否写入KV日志 : false; // HARDENED: logging disabled by default. Set ON_LOG=1 to enable.
		if (!是否写入KV日志) return;
		let 日志数组 = [];
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数) {
	...
	try {
		...
		const tcpSocket = 代理协议 === 'socks5'
			? await socks5Connect('speed.cloudflare.com', 80, initialData)
			: (代理协议 === 'https'
				? await httpConnect('speed.cloudflare.com', 80, initialData, true)
				: await httpConnect('speed.cloudflare.com', 80, initialData));
